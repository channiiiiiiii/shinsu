...
- 앱 0.3.0 / 신규 게임 저장 스키마 2. 미래 버전은 오류 처리하며 초기화하지 않는다.
...
개인·협동 레이드, 강화·별 각성·잠재 성장·환생, 신화·고대 던전, 성장 스킬과 조건부 각인 효과를 제공한다. 협동 대기방은 15분 유효하며 참가 후 두 계정이 각각 스킬을 고르면 한 턴씩 진행한다. 턴 상태는 SQLite에 저장되고 승패가 결정되면 두 계정을 한 트랜잭션으로 정산한다. 포기 시 참가자 기력만 소모한다. 패배도 저장되며, 전투는 최대 100턴이다. 실제 운영 계정의 수동 플레이 검증은 사용자 수행 범위다.

Supabase 백업과 기존 저장 이전의 설정·명령은 [확장 기능 운영 안내](EXPANSION.md)를 참고한다.
